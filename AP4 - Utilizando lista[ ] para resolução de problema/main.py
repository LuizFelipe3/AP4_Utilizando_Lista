...
"""
Luiz Felipe RA 006108 / 5 Semestre
...
def getConsumoCliente(num_cliente: int) -> int:
    ...
    consumo = int(input(f'Qual o consumo de água do cliente {num_cliente}?\n>>> '))
    return consumo

def startLeituras(num_leituras: int) -> list[int]:
    """Começar a rotina da leitura de água."""
    leitura_list = []
...
def calcMediaLeituras(list_leituras: list[int]) -> float:
    """Calcular a media de todas as leituras."""
    soma = 0
